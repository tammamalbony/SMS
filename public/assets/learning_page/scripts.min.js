...
(() => { // webpackBootstrap
  ...
  (function ($) {
    ...
    function handleDownloadCertificateHtml(result) {
      ...
      var otherHtml = '';
      if (result && result !== '') {
        otherHtml = "\n                <a href=\"/panel/quizzes/results/".concat(result, "/showCertificate\" target=\"_blank\" class=\"btn btn-primary btn-sm mt-15\">").concat(showLang, "</a>\n            ")+ "\n                <a href=\"/panel/quizzes/results/".concat(result, "/downloadCertificate\" class=\"btn btn-primary btn-sm mt-15\">").concat(downloadLang, "</a>\n            ");
      } else {
    ...
    $('#nav-tabContent').on('click', '.js-cant-start-quiz-toast', function (e) {
      e.preventDefault();
      if (cantStartQuizToastTitleLang && cantStartQuizToastMsgLang) {
        errorToast(cantStartQuizToastTitleLang, cantStartQuizToastMsgLang);
      }
    ...
    document.addEventListener('DOMContentLoaded', function () {
      var navbarCloseButton = document.getElementById('navbarClose');
      var learningPageTabs = document.querySelector('.learning-page-tabs');

      navbarCloseButton.addEventListener('click', function () {
        learningPageTabs.classList.remove('show');
      });
    });

    document.addEventListener('DOMContentLoaded', function () {
      var navbarCloseButton = document.getElementById('collapseBtn2');
      var learningPageTabs = document.querySelector('.learning-page-tabs');

      navbarCloseButton.addEventListener('click', function () {
        learningPageTabs.classList.add('show');
      });
    });
    document.addEventListener('DOMContentLoaded', function () {
      // Check if the URL contains the word "assignments"
      if (!window.location.href.includes("assignment")) {
        const contentTab = document.querySelector('.learning-page-tabs #nav-tabContent .content-tab');
        if (contentTab) {
          const uncheckedElements = contentTab.querySelectorAll('.js-passed-lesson-toggle:not(:checked)');
          if (uncheckedElements.length > 0) {
            const firstUncheckedElement = uncheckedElements[0];
            const accordionRow = firstUncheckedElement.closest('.accordion-row');
            if (accordionRow) {
              accordionRow.click();
              const collapseButton = accordionRow.querySelector('[data-toggle="collapse"]');
              if (collapseButton) {
                collapseButton.click();
                const tabItem = firstUncheckedElement.closest('.tab-item');
                if (tabItem) {
                  tabItem.click();
                  setTimeout(function () {
                    const container = document.querySelector('.content-tab');
                    const containerRect = container.getBoundingClientRect();
                    const tabItemRect = tabItem.getBoundingClientRect();
                    const scrollTop = tabItemRect.top - containerRect.top - (containerRect.height / 2) + (tabItemRect.height / 2);
                    container.scrollTo({
                      top: scrollTop,
                      behavior: 'smooth'
                    });
