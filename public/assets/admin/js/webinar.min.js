    ...
    $.get(adminPanelPrefix + '/filters/get-by-category-id/' + category_id, function (result) {
        if (result && typeof result.filters !== "undefined" && result.filters.length) {
            ...
            Object.keys(result.filters).forEach(function (key) {
                ...
                const filterTitle = getLocalizedTitle(filter.translations, $('#webniare_creat_lang').val());
                console.log("Seletected lang :"+$('#webniare_creat_lang').val());
                ...
                if (options.length) {
                    Object.keys(options).forEach(function (index) {
                        ...
                        const optionTitle = getLocalizedTitle(option.translations, $('#webniare_creat_lang').val());

                        html += '<div class="form-group mt-20 d-flex align-items-center justify-content-between">\n' + 
                                '<label class="" for="filterOption' + option.id + '">' + optionTitle + '</label>\n' + // Update here!
...
function getLocalizedTitle(translations, locale) {
  locale = locale.toLowerCase(); // Convert locale to lowercase
  const localizedTitle = translations.find(t => t.locale.toLowerCase() === locale)?.title;
  if (localizedTitle) {
    console.log("Found the needed translation");
      return localizedTitle;
  } else {
      const arabicTitle = translations.find(t => t.locale.toLowerCase() === 'ar')?.title;
      if (arabicTitle) {
          console.log("Found the fallback ar translation");
          return arabicTitle;
      } else {
          const englishTitle = translations.find(t => t.locale.toLowerCase() === 'en')?.title;
          console.log("Found the fallback en translation");
          return englishTitle || 'Untitled Title';
      }
  }
}
  /**
  ...
  $(document).ready(function () {
    ...
    function updateToDatabase(table, idString) {
      ...
      // Check if the URL is for forms or webinars
      if (baseUrl.includes('/admin/forms/')) {
        const formId = baseUrl.split('/admin/forms/')[1].split('/')[0]; 
        postUrl = adminPanelPrefix + `/forms/${formId}/fields/orders`;
      } else if (baseUrl.includes('/admin/webinars/')) {
        postUrl = adminPanelPrefix + '/webinars/order-items';
      } else {
        // Default or error handling
        console.error('Unsupported URL for this operation');
        return;  
      }
    
      // Post data to the determined URL
      $.post(postUrl, {
        table: table,
        items: idString
      }, function (result) {
  ...
  $('body').on('click', '.js-add-course-content-btn, .add-new-interactive-file-btn', function (e) {
    ...
    var html = '';
    console.log("start add course content btn : type : " + type + "  || chapterId :" + chapterId + "  || contentTagId " +contentTagId+  " || key : " +key );

    switch (type) {
      case 'file':
        console.log("Starting 'file' case execution.");
        console.log("Start add File Form");
    
        var newFileForm = $('#newFileForm');
        console.log("New file form:", newFileForm);
    
        console.log("Setting chapter input value to:", chapterId);
        newFileForm.find('.chapter-input').val(chapterId);
    
        var html = newFileForm.html();
        console.log("HTML content of the new file form:", html);
    
        html = html.replace(/record/g, key);
        console.log("HTML content after replacing 'record' with key:", html);
    
        $(contentTagId).prepend(html);
        console.log("Prepending modified HTML content to", contentTagId);
        break;
  ...
  * */
  ...
  $('body').on('click', '#add_new_learn_features', function (e) {
    e.preventDefault();
    var key = randomString();
    var html = '<div id="extraDescriptionModal">';
    html += $('#extraDescriptionForm').html();
    ...
    Swal.fire({
      ...
      width: '48rem',
      onOpen: function onOpen() {
        $('#extraDescriptionModal input[name="type"]').val('learn_features');
      }
    });
  });
  ...
  * */
   ...
   $('body').on('click', '#add_new_vip_features', function (e) {
    e.preventDefault();
    var key = randomString();
    var html = '<div id="extraDescriptionModal">';
    html += $('#extraDescriptionForm').html();
    ...
    Swal.fire({
      ...
      width: '48rem',
      onOpen: function onOpen() {
        $('#extraDescriptionModal input[name="type"]').val('vip_features');
      }
    });
  });
 /*
 ...
 $('body').on('click', '#add_new_course_features', function (e) {
  e.preventDefault();
  var key = randomString();
  var html = '<div id="extraDescriptionModal">';
  html += $('#extraDescriptionForm').html();
  ...
  Swal.fire({
    ...
    width: '48rem',
    onOpen: function onOpen() {
      $('#extraDescriptionModal input[name="type"]').val('course_features');
    }
  });
});
